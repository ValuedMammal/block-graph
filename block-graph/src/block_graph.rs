...
impl<T: ToBlockHash + fmt::Debug + Ord + Clone> BlockGraph<T> {
    ...
    pub fn from_changeset(changeset: ChangeSet<T>) -> Result<Option<Self>, MissingGenesisError> {
        ...
        let prev_hashes: HashMap<BlockId, BlockHash> = changeset
            .blocks
            .iter()
            .map(|(id, (_, parents))| (*id, parents.iter().last().expect("must have parent").hash))
            .collect();
        ...
        loop {
            match (original_tip.peek(), update_tip.peek()) {
                ...
                (Some(original), Some(update)) => {
                    ...
                    // a few scenarios:
                    //
                    // 1. The checkpoint definitely has a `prev`.
                    // 2. else we need to locate the next nearest block of the *original chain*
                    //    that sits at a lower height than the update block.
